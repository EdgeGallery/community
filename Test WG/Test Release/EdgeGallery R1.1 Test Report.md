...
已剔除 14个无效bug后，总缺陷数量： 262（含 34个安全类缺陷），已解决数量： 264个；未解决bug 12个（含12 个known issue，遗留到下一版本解决）。

3.3.2 缺陷分析 Defects Analysis

按缺陷类型分析：安全类bug 35个；其他类bug 241个。
按缺陷严重级别分析：严重bug 31个；主要bug 68个；次要bug 127个；不重要bug 9个；不指定41 个。
按需求点分析
缺陷收敛性分析：总计276 个bug（含 14个无效bug；含安全类bug 34）；迭代一 39 个bug（含 3个无效bug）；迭代二 65 个bug（含 5个无效bug）；迭代三 143 个bug（含 4个无效bug）；未制定迭代29 个bug（含 2个无效bug）
...
|--|--|--|--|--|
| I3B5TC |【appstore】建议通过浏览器返回到首页记住之前所在位置  |遗留 | 属于优化问题，下个版本再做 |  |
| I3CAM5 |【mecm】优化建议：建议“应用包管理”列表中增加“应用市场”和“同步时间”字段 |遗留 | 暂时接口无对应信息，无法添加，可以下个版本优化|  |
| I3D6SN |【Developer】部署调测monitoring-service-mepagent的项目时，界面显示部署成功，按其后台pod未启动成功 |遗留 | 建议下一版本在developer部署调测页面里当一个应用实例化后“部署成功”可以改成其他文字，以体现应用helm部署成功但应用镜像可能拉取失败的情况|  |
| I38U3X |【UserMgmt】使用不存在的账号登录，失败5次后无有效阻止措施 |遗留 | 经讨论，该问题遗留到下个版本，强化验证方式，通过能与服务端进行联动的验证方式来实现验证|  |
