...
已剔除 9个无效bug后，总缺陷数量： 202（含 36个安全类缺陷），已解决数量：178 个；未解决bug 24个（含6 个known issue，遗留到下一版本解决）。

3.3.2 缺陷分析 Defects Analysis

按缺陷类型分析：安全类bug 38个；其他类bug 173个。
按缺陷严重级别分析：严重bug 18个；主要bug 71个；次要bug 86个；不重要bug 10个；不指定26 个。
按需求点分析
缺陷收敛性分析：总计211 个bug（含 9个无效bug；含安全类bug 38）；迭代一 22 个bug（含 1个无效bug）；迭代二 85 个bug（含 5个无效bug）；迭代三 78 个bug（含 2个无效bug）；未制定迭代 26 个bug（含 1个无效bug）
