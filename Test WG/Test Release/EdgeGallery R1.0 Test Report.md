...
已剔除 9个无效bug后，总缺陷数量： 202（含 35个安全类缺陷），已解决数量：195 个；未解决bug 7个（含7 个known issue，遗留到下一版本解决）。
...
- 需求完成率（EPIC）20/31=65%，（story）93/111=84%, 尚未达到版本发布质量要求。
- 为解决严重问题<=1,满足版本发布质量要求。
- 遗留DI=2*0.1+5*1+1*3=8.2<10，满足版本发布质量要求。
