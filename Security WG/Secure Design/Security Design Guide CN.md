...
4.6 确定密码重置功能可防止猜测或绕过。
1.如果应用程序允许在不请求当前密码的情况下更改密码。确实，如果攻击者能够控制有效会话，则他们可以轻松更改受害者的密码，验证重置密码时候是否需要输入当前密码。
2.建议：重置密码功能是否在更改密码之前要求确认， 应用程序应使用随机令牌通过电子邮件将链接发送给用户，并且仅当用户访问该链接时，重置过程才完成。这样可以确保在确认重置之前，当前密码仍然有效
...
**注意** ：研究源码分析解锁机制可能存在的漏洞，比如是否存在拒绝服务攻击，而使得账户一直得不到解锁。
