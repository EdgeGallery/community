...
| 相关API          | jwt生成token接口<br>从dev上传csar包到app<br>mecm从app拉取csar包|
| API有效负载示例     | 接口样例（待确定）：<br>{    <br>    "authInfo":{<br>        "credentials":{<br>            "accessKeyId":"QVUJMSUMgS0VZLS0tLS0",<br>           "secretKey":"DXPb4sE1KhcHe07Kw5uorayETwId1JOjjOIRomRs5wyszoCR5R7AtVa28KT31Sc"<br>&nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp;          }<br>    &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; }<br>}|
| 相关文件           | 不涉及 |
| 相关GUI URL        |   待确定                        |
...
| 实施细节   | **user修改密码，修改用户信息等接口排查，需要防止横向越权。app/dev代码中有对用户角色的校验(使用@PreAuthorize进行提权访问)，需要识别出有横向越权的接口。**  |
| 相关API          |待识别 |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |   不涉及                        |
...
| 实施细节   |   **user/app/dev未处理，需要创建非root账号，启动镜像，mecm待分析**|
| 相关API          |不涉及 |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        不涉及                   |
...
| 实施细节   | **user模块忘记密码采用手机认证，无token禁用机制，无更新配置文件，app/dev/mecm模块不涉及**  |
| 相关API          |user(PUT): /v1/users/password |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        /v1/users/password                  |
| GUI测试步骤      | 1.进入appstore或者dev或者mecm的登录页面，点击忘记密码，挑战至重置密码页面<br>2.填写正确的手机号，获取验证码，填写新密码和密码确认<br>3. 点击提交按钮|
| GUI输入示例       |     不涉及        |
| 附加信息         | 不涉及 |
### 1.7 所有身份验证控件必须安全失败。
...
| 实施细节   | **user模块需要排查日志中的敏感数据，排查登录失败的模糊提示语。app和dev模块token校验失败后需要增加日志，mecm模块待分析**  |
| 相关API          |不涉及 |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        不涉及                   |
...
| 实施细节   | **user模块当前通过手机验证码重置密码，app/dev/mecm不涉及**  |
| 相关API          |user(PUT): /v1/users/password |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        /v1/users/password                   |
...
| 实施细节   | **user模块已采取密码复杂度校验，app/dev模块不涉及，mecm待分析**  |
| 相关API          |不涉及 |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        不涉及                   |
...
| 实施细节   | **user/app/dev所有接口采用https单向认证,mecm待分析**  |
| 相关API          |不涉及 |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        不涉及                   |
...
| 实施细节   | **user/app/dev模块目前开启了SSL协议，但是未做版本限制，mecm模块待分析**  |
| 相关API          |不涉及 |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        不涉及                   |
...
| 实施细节   | **user/app/dev模块目前版本未做证书管理功能，不涉及吊销功能，mecm待分析**  |
| 相关API          |不涉及 |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        不涉及                   |
...
| 实施细节   | **user-mgmt启动时，jwt公钥、私钥和私钥密码通过环境变量传入，用户密码使用不可逆加密存储，数据库密码和redis密码是通过环境变量的明文传入的，不存在用户通过配置文件导入密码的场景，app/dev/mecm身份验证均存在数据库，以不可逆PBKDF2加密存储**  |
| 相关API          |不涉及 |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        不涉及                   |
...
| 实施细节   | **user模块用户登录密码通过不可逆加密后存储在数据库，app/dev模块启动时https证书密码明文传入，明文传入的密码存在于环境变量，使用完成后需要做清空处理，mecm模块待分析**  |
| 相关API          |不涉及 |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        不涉及                   |
...
| 实施细节   | **user/app/dev模块的敏感数据均通过请求体body传入，mecm待分析**  |
| 相关API          |不涉及 |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        不涉及                   |
...
| 实施细节   | **user/app/dev模块无数据采集，不收集个人数据。当前的注册信息有手机号、公司、邮箱、性别，需要提供免责声明说明，用户勾选后才能继续注册，mecm模块待分析**  |
| 相关API          |不涉及 |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        不涉及                   |
...
| 实施细节   | **user/dev模块不涉及，app模块认证测试过程中的病毒扫描需要给第三方提供接口，存在将csar包发给第三方的风险，需要考虑授权，mecm模块待分析**  |
| 相关API          |不涉及 |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        不涉及                   |
...
| 实施细节   | **user/app/dev模块未使用非标准的加密算法，使用的有PBKDF2、Java KeyStore等算法，mecm模块待分析**  |
| 相关API          |不涉及 |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        不涉及                   |
...
| 实施细节   | **user模块用户密码使用的是不可逆的PBKDF2，数据库、redis等密码都是通过环境变量传入的，需要注意的是在1.0版本appstore模块的csar包签名算法要使用安全的加密算法，dev模块不涉及，mecm模块待分析**  |
| 相关API          |不涉及 |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        不涉及                   |
...
| 实施细节   | **user/app/dev模块数据库、jwt、证书等的秘钥是通过环境变量传入，系统内没有使用加密算法加密数据，不存在硬编码，mecm模块待分析**  |
| 相关API          |不涉及 |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        不涉及                   |
...
| 实施细节   | **user模块已使用加密安全的随机数SecureRandom，app/dev/mecm模块待分析**  |
| 相关API          |不涉及 |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        不涉及                   |
...
| 实施细节   | **user/app/dev模块已满足（使用安全的随机数SecureRandom），mecm模块待分析**  |
| 相关API          |不涉及 |
| API有效负载示例     | 不涉及|
| 相关文件           | 不涉及 |
| 相关GUI URL        |        不涉及                   |
