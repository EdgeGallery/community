...
| 实施细节   | **$\color{red}{当前版本此设计需要确认}$<br>待排查user/app/dev的日志记录中是否有未经消毒的用户输入**  |
...
| 实施细节   | **$\color{red}{当前版本此设计需要确认}$<br>user/app/dev中所有接口，均有日志记录，需要在日志记录中增加访问的客户端IP，mecm模块待分析**  |
| 相关API          |user/app/dev平台所有的api |
| API有效负载示例     | 不涉及|
| 相关文件           | 基于日构建环境，日志文件位置<br>user: /var/log/user-mgmt/user-mgmt-be.log <br>app: /var/log/appstore/appstore-be.log <br>dev: /var/log/developer/developer-be.log |
| 相关GUI URL        |        不涉及                   |
...
| 实施细节   | **$\color{red}{当前版本此设计需要确认}$<br>待排查user/app/dev/mecm各模块的日志记录中是否含有密码、密钥、session、token等敏感信息**  |
| 相关API          |user/app/dev平台所有的api |
| API有效负载示例     | 不涉及|
| 相关文件           | 基于日构建环境，日志文件位置<br>user: /var/log/user-mgmt/user-mgmt-be.log <br>app: /var/log/appstore/appstore-be.log <br>dev: /var/log/developer/developer-be.log |
| 相关GUI URL        |        不涉及                   |
